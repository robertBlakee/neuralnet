...
async function ifeModel_loadLocal(){
	...
	mobilenet = await tf.loadGraphModel( tf.io.browserFiles( [IFEMODEL_JSON_INPUT.files[0], IFEMODEL_BIN_INPUT.files[0]] ));
...
async function imageData_save(){
	...
	await trainingDataOutputs.save( 'downloads://savedData' );
}
