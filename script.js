...
const IFEMODEL_JSON_INPUT 			= document.getElementById('ifeModel_json_input');
const IFEMODEL_BIN_INPUT 				= document.getElementById('ifeModel_bin_input');
const CMODEL_JSON_INPUT 				= document.getElementById('cModel_json_input');
const CMODEL_BIN_INPUT 					= document.getElementById('cModel_bin_input');
...
async function ifeModel_loadWeb() {
  ...
  //Save this mobilnet model on local
  await mobilenet.save( 'downloads://ifeModel_1' );
  //await mobilenet.save( 'indexeddb://ifeModel_1' );
}
...
async function ifeModel_loadLocal(){
	...
	mobilenet = await tf.loadGraphModel( tf.io.browserFiles( [IFEMODEL_JSON_INPUT.files[0], IFEMODEL_JSON_INPUT.files[1]] ));
	//mobilenet = await tf.loadGraphModel( 'indexeddb://ifeModel_1' );
  STATUS.innerText = 'MobileNet v3 loaded successfully!';
...
async function cModel_load(){
	...
	model = await tf.loadLayersModel( tf.io.browserFiles( [CMODEL_JSON_INPUT.files[0], CMODEL_BIN_INPUT.files[0]] ) );
...
async function cModel_save(){
	...
	await model.save( 'downloads://cModel_1' );
}

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Load image data - button handler
function imageData_load(){
	//if( (model == undefined) || predict ) return;
	
	console.log( IMAGEDATA_INPUT.files[0] );
	let variable = IMAGEDATA_INPUT.files[0];
	console.log( readAsText(variable) );
}
    ...
    tf.tidy(function() {
      ...
      if( predictionArray[ highestIndex ] >= 0.97 ){
      	switch( highestIndex ){
      		...
      		case 2:		client.publish( 'SRP_topic', 'b' );	break;
      		case 3:		client.publish( 'SRP_topic', 'l' );	break;
      		case 4:		client.publish( 'SRP_topic', 'r' );	break;
      		case 5:		client.publish( 'SRP_topic', 't' );	break;
      	}
      }
    });
...
// Purge data and start over. Note this does not dispose of the loaded MobileNet model and MLP head tensors
// as you will need to reuse them to train a new model.
function reset() {
  predict = false;
