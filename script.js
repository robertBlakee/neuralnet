...
async function ifeModel_loadLocal(){
	...
	mobilenet = await tf.loadGraphModel( tf.io.browserFiles( [IFEMODEL_INPUT.files[0], IFEMODEL_INPUT.files[1]] ));
...
async function cModel_load(){
	...
	model = await tf.loadLayersModel( tf.io.browserFiles( [CMODEL_INPUT.files[0], CMODEL_INPUT.files[1]] ) );
...
async function imageData_load(){
	...
	for( let j = 0; j < trainingDataOutputs.length; j++ ){
		...
		tensor_asArray = savedData.trainingDataInputs_asArray[ j ];
		trainingDataInputs.push( tf.tensor( tensor_asArray ));
...
async function imageData_save(){
	...
	let trainingDataInputs_asArray = [];
	let tensor;
	
	for( let j = 0; j < trainingDataOutputs.length; j++ ){
		tensor = trainingDataInputs[ j ];
		trainingDataInputs_asArray.push( tensor.arraySync() );
	}
	
	let dataToSave = { trainingDataOutputs, trainingDataInputs_asArray };
	saveFile( dataToSave );
