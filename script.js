...
const TESTDATA_PREDICT_BUTTON 	= document.getElementById('testData_load');
...
const TESTDATA_INPUT 						= document.getElementById('testData_input');
...
TESTDATA_PREDICT_BUTTON.addEventListener('click', testData_predict );
...
async function imageData_load(){
	...
	STATUS.innerText = 'Training image data loading ...';
...
// Read image source url from file
async function readFile_imgSrc( file ){
  const reader = new FileReader();
...
// Read image from file (return image object)
async function readFile_image( file ){
  let imageSource = await readFile_imgSrc( file );
	let img = new Image();
	img.src = imageSource;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//Make predictions on test images - button handler
async function testData_predict(){
	ifeModel 	= await tf.loadGraphModel( tf.io.browserFiles(
	 [IFEMODEL_INPUT.files[0], IFEMODEL_INPUT.files[1]] ));
	cModel 		= await tf.loadLayersModel( tf.io.browserFiles(
	 [CMODEL_INPUT.files[0], CMODEL_INPUT.files[1]] ) );
	...
	for( let j=0; j<TESTDATA_INPUT.files.length; j++ ){
		//image loading
		let image = await readFile_image( TESTDATA_INPUT.files[ j ] );
		image.height = 360;
		image.width  = 480;
		
		//this block is wrap in "setTimeout()" becasue of incorrect predictions
		setTimeout( function(){
    	...
    	Results.object.push( CLASS_NAMES[ highestIndex ] );
    	Results.score.push( Math.floor( predictionArray[highestIndex]*100 ) );
    	Results.time.push( stopTime - startTime );
    	
    	console.log( Results.object[ j ] + ': '+ Results.score[ j ] +
    	' %   [' + Results.time[ j ] + ' ms]' );
		}, 0); 
	}
	
	setTimeout( function(){	
		saveFile( Results.object );
		saveFile( Results.score );
		saveFile( Results.time );
	}, 0); 
}
